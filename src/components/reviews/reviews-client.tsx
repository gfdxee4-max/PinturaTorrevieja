...
function errorMessage(copy: ReviewCopy, code?: string) {
  ...
  if (code === "PAYLOAD_TOO_LARGE") return copy.errors.invalid;
      ...
      <div className="mx-auto max-w-[90rem] px-5 py-16 sm:px-8 lg:px-12 lg:py-20">
        ...
        {averageRating !== null ? (
          <div className="relative mx-auto mt-6 flex w-fit items-center gap-5 overflow-hidden rounded-[5px] border border-white/20 bg-[#090909]/95 px-6 py-4">
            ...
            <div className="flex gap-1" role="img" aria-label={`${formattedRating} ${ui.ratingOutOfFive}`}>
            ...
            {visibleReviews.map((review) => (
              <article key={review.id} className="flex min-h-[17rem] flex-col rounded-[6px] border border-white/20 bg-[#090909]/95 p-6 transition duration-300 hover:border-white/38">
                <div className="grid grid-cols-[3.5rem_1fr_auto] items-start gap-4">
                  ...
                  <div><h3 className="text-lg font-semibold text-white">{review.name}</h3><div className="mt-1 flex gap-1" role="img" aria-label={`${copy.rating}: ${review.rating} ${ui.ratingOutOfFive}`}>{Array.from({ length: 5 }, (_, index) => <Star key={index} className={`size-5 ${index < review.rating ? "fill-redline text-redline" : "text-white/22"}`} strokeWidth={1.5} aria-hidden="true" />)}</div></div>
          ...
          <div>
            ...
            <form id="review-form" className="scroll-mt-28 mt-6 grid gap-3" onSubmit={submitReview} noValidate>
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <fieldset className="flex min-h-12 items-center gap-0 rounded-[3px] border border-white/24 bg-black/55 px-2 sm:px-3"><legend className="sr-only">{copy.rating}</legend>{[1,2,3,4,5].map((value) => <button key={value} type="button" aria-label={ui.ratingAction.replace("{rating}", String(value))} aria-pressed={value <= rating} onClick={() => setRating(value)} className="flex size-11 shrink-0 items-center justify-center text-redline focus-visible:outline-white"><Star className={`size-5 ${value <= rating ? "fill-current" : "fill-transparent"}`} strokeWidth={1.5} aria-hidden="true" /></button>)}<span className="ml-2 hidden text-sm text-white/50 xl:inline">{copy.rating} *</span><input type="hidden" name="rating" value={rating} required /></fieldset>
              </div>
              ...
              <label className="flex items-start gap-3 text-sm leading-5 text-white/62"><input required type="checkbox" name="consent" className="mt-0.5 size-5 shrink-0 accent-[#d60000]" /><span>{copy.consent} <Link className="text-[#ed3333] underline-offset-4 hover:underline" href={`/${locale}/privacy`}>{copy.privacy}</Link> *</span></label>
